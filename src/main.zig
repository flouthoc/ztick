...
fn add_note(stack: *gtkc.GtkWidget) callconv(.C) void {
    ...
    _ = gtkc.gtk_stack_add_titled(@ptrCast(*gtkc.GtkStack, stack), @ptrCast(*gtkc.GtkWidget, textbox_new), str.ptr, str.ptr);
    _ = gtkc.g_signal_connect_data(textbox1_buffer, "changed", @ptrCast(gtkc.GCallback, &write_note), textbox1_buffer, null, gtkc.G_CONNECT_SWAPPED);
...
fn delete_note(stack: *gtkc.GtkWidget) callconv(.C) void {
    const widget = gtkc.gtk_stack_get_visible_child(@ptrCast(*gtkc.GtkStack, stack));
    const textbox1_buffer = gtkc.gtk_text_view_get_buffer(@ptrCast(*gtkc.GtkTextView, widget));
...
fn write_note(textbufholder: *gtkc.GtkTextBuffer) callconv(.C) void {
    var textbuffer = @ptrCast(*gtkc.GtkTextBuffer, textbufholder);
...
fn on_activate(app: *gtkc.GtkApplication, data: gtkc.gpointer) callconv(.C) void {
    ...
    gtkc.gtk_css_provider_load_from_data(@ptrCast(*gtkc.GtkCssProvider, css_provider), css_data, css_data.len);
    gtkc.gtk_css_provider_load_from_data(@ptrCast(*gtkc.GtkCssProvider, css_provider_create), css_data_create_note, css_data_create_note.len);
    const window = gtkc.gtk_application_window_new(app);
    gtkc.gtk_window_set_default_size(@ptrCast(*gtkc.GtkWindow, window), 400, 300);
    gtkc.gtk_window_set_title(@ptrCast(*gtkc.GtkWindow, window), "ztick - tiny notes utility");
    gtkc.gtk_window_set_resizable(@ptrCast(*gtkc.GtkWindow, window), 1);
    const button = gtkc.gtk_button_new_with_mnemonic("Create new note");
    const context = gtkc.gtk_widget_get_style_context(@ptrCast(*gtkc.GtkWidget, button));
    _ = gtkc.gtk_style_context_add_provider(context, @ptrCast(*gtkc.GtkStyleProvider, css_provider_create), 800);
    const button_delete = gtkc.gtk_button_new_with_mnemonic("Delete this note");
    const context_delete_button = gtkc.gtk_widget_get_style_context(@ptrCast(*gtkc.GtkWidget, button_delete));
    _ = gtkc.gtk_style_context_add_provider(context_delete_button, @ptrCast(*gtkc.GtkStyleProvider, css_provider), 800);
    const stack = gtkc.gtk_stack_new();
    const textbox1 = gtkc.gtk_text_view_new();
    const textbox1_buffer = gtkc.gtk_text_view_get_buffer(@ptrCast(*gtkc.GtkTextView, textbox1));
    ...
    _ = gtkc.gtk_stack_add_titled(@ptrCast(*gtkc.GtkStack, stack), @ptrCast(*gtkc.GtkWidget, textbox1), "Page1", "Page1");
    _ = gtkc.gtk_stack_sidebar_set_stack(@ptrCast(*gtkc.GtkStackSidebar, stack_sidebar), @ptrCast(*gtkc.GtkStack, stack));
    ...
    var size_u: c_int = 0;
    size_u = @intCast(c_int, @truncate(usize, bytes_read));
    ...
    while (i <= max_notes) {
        ...
        size_u_local = @intCast(c_int, @truncate(usize, bytes_read_local));
    ...
    gtkc.gtk_paned_set_start_child(@ptrCast(*gtkc.GtkPaned, hpaned), button_delete);
    gtkc.gtk_paned_set_resize_start_child(@ptrCast(*gtkc.GtkPaned, hpaned), 0);
    gtkc.gtk_paned_set_shrink_start_child(@ptrCast(*gtkc.GtkPaned, hpaned), 0);
    gtkc.gtk_widget_set_size_request(button_delete, 40, -1);

    gtkc.gtk_paned_set_end_child(@ptrCast(*gtkc.GtkPaned, hpaned), button);
    gtkc.gtk_paned_set_resize_end_child(@ptrCast(*gtkc.GtkPaned, hpaned), 0);
    gtkc.gtk_paned_set_shrink_end_child(@ptrCast(*gtkc.GtkPaned, hpaned), 0);
    gtkc.gtk_widget_set_size_request(button, 20, -1);

    const box_main = gtkc.gtk_box_new(1, 1);
    const box_set = gtkc.gtk_box_new(0, 0);
    _ = gtkc.gtk_box_append(@ptrCast(*gtkc.GtkBox, box_set), stack_sidebar);
    _ = gtkc.gtk_box_append(@ptrCast(*gtkc.GtkBox, box_set), stack);
    _ = gtkc.gtk_box_append(@ptrCast(*gtkc.GtkBox, box_main), box_set);
    _ = gtkc.gtk_box_append(@ptrCast(*gtkc.GtkBox, box_main), hpaned);

    gtkc.gtk_window_set_child(@ptrCast(*gtkc.GtkWindow, window), box_main);
    gtkc.gtk_window_present(@ptrCast(*gtkc.GtkWindow, window));
}

pub fn main() !void {
    const app = gtkc.gtk_application_new("com.github", gtkc.G_APPLICATION_FLAGS_NONE);
    defer gtkc.g_object_unref(app);
    _ = gtkc.g_signal_connect_data(app, "activate", @ptrCast(gtkc.GCallback, &on_activate), null, null, 0);
    _ = gtkc.g_application_run(@ptrCast(*gtkc.GApplication, app), 0, null);
}
